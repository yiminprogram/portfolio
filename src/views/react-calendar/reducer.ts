...
const createCurrentMonthList = (
  ...
  while (temp.getMonth() === firstDate.getMonth()) {
    const data = dataBase.filter(
      (ele) => ele.id.toLocaleDateString() === temp.toLocaleDateString(),
    );
    arr.push({
      id: new Date(temp),
      list: [...data],
...
export const prevMonth = (state: TCalendarContext): TCalendarContext => {
  ...
  const boardList = state.dataBase.filter(
    (ele) => ele.id.toLocaleDateString() === tempCurrent.toLocaleDateString(),
  );
...
export const backToday = (state: TCalendarContext): TCalendarContext => {
  ...
  const boardList = state.dataBase.filter(
    (ele) => ele.id.toLocaleDateString() === new Date().toLocaleDateString(),
  );
